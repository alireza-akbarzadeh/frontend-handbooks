...
const InstructionItem: React.FC<{
  item: InstructionStep | LearningPathItem;
  isPathItem?: boolean;
}> = ({ item, isPathItem = false }) => {
  ...
  return (
    <li
      className={cn(
        ...
        isPathItem
          ? "border-b border-slate-200 dark:border-slate-700 last:border-0 pb-4 last:pb-0"
          : "",
      )}
    >
      {Icon && (
      ...
      <div>
        <div className="flex items-center">
          <span
            className={cn("font-medium", isPathItem ? "text-base" : "text-sm")}
          >
            {item.title}
          </span>
          {item.badge && (
